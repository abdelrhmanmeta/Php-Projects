...
function save_doctor(){
		...
		$data .= ", major = '$major' ";
		if(empty($id)){
			$save = $this->db->query("INSERT INTO doctor_list set ".$data);
		}else{
			$save = $this->db->query("UPDATE doctor_list set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_doctor(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM doctor_list where id = ".$id);
		if($delete)
			return 1;
	}
